...
// The BACKGROUND_IMAGE_URL has been removed as requested.
...
const SplashScreen = ({ onFadeOut }) => {
    ...
    useEffect(() => {
        // Increased wait time to 2.5 seconds to help prevent premature 404s
    ...
    return (
        ...
        >
            <img 
                ...
                onError={(e) => { e.target.onerror = null; e.target.src=`https://placehold.co/300x400/111827/ffffff?text=LOGO+FAIL`; console.error("Logo image failed to load. Check Content Fetch ID."); }}
            />
        </div>
    );
};
      ...
      <div className={`text-center mb-12 py-10 bg-${CARD_DARK}/70 rounded-2xl shadow-2xl backdrop-blur-sm border-b-4 border-${ACCENT_RED}`}>
        ...
        <div className="mb-8 p-4">
          <img 
            ...
            onError={(e) => { e.target.onerror = null; e.target.src=`https://placehold.co/300x100/1f2937/ffffff?text=EVENT+NAME+FAIL`; console.error("Event Name image failed to load. Check Content Fetch ID."); }}
          />
        </div>
...
export default function App() {
  ...
  return (
    // Set the solid dark background color directly on the main container
    <div className={`min-h-screen font-sans text-white bg-${BG_DARK}`}>
      {/* 1. SPLASH SCREEN */}
