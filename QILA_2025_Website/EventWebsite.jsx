...
import { Home, Trophy, BarChart3, Image, Phone, Speaker, Menu, X, Calendar, MapPin, Heart, Settings, Edit3, Save, Loader2, Zap } from 'lucide-react';
...
// !!! IMPORTANT: These URLs must point to the files you upload to your hosting environment's public folder.
const LOGO_IMAGE_URL = "/LOGO.jpeg";
const EVENT_NAME_IMAGE_URL = "/EVENT NAME.jpeg";
const BACKGROUND_IMAGE_URL = "/BAGROUND.jpg";

// Event details for text elements
const EVENT_NAME = "QILA 2025"; // UPDATED NAME
...
const SplashScreen = ({ onFadeOut }) => {
    ...
    useEffect(() => {
        // Show logo for 2 seconds, then fade out over 1 second
        const timer = setTimeout(() => {
            setIsFaded(true);
            const fadeTimer = setTimeout(onFadeOut, 1000); // 1000ms is the CSS transition duration
    ...
    return (
        ...
        >
            <img 
                ...
                onError={(e) => { e.target.onerror = null; console.error("Logo image not found at path:", LOGO_IMAGE_URL); }}
            />
        </div>
    );
};
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      <div className={`text-center mb-12 py-10 bg-${CARD_DARK}/70 rounded-2xl shadow-2xl backdrop-blur-sm`}>
        ...
        {/* Main Logo Area (Small version for persistent view) */}
        <div className="mb-8">
          <img 
            src={LOGO_IMAGE_URL} 
            alt="Event Logo" 
...
// --- SCOREBOARD COMPONENT (PUBLIC VIEW) ---
        ...
        const unsubscribe = onSnapshot(docRef, (docSnapshot) => {
            if (docSnapshot.exists()) {
                ...
                if (Array.isArray(data.colleges)) {
                    ...
                    // Initialize if data structure is missing
                    setStandingsData(INITIAL_COLLEGE_SETUP);
                }
            } else {
                // Document doesn't exist, use initial setup
                setStandingsData(INITIAL_COLLEGE_SETUP);
            }
            setLoading(false);
        }, (error) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnapshot) => {
            if (docSnapshot.exists()) {
                ...
                // If doc doesn't exist, set initial setup for first save
        ...
        setStandings(prevStandings =>
            prevStandings.map(college => {
                if (college.id === collegeId) {
                    ...
                    const newTotal = Object.values(newPoints).reduce((sum, p) => sum + p, 0);
                    return { ...college, points: newPoints, total: newTotal };
                }
                return college;
            })
        );
        ...
        return (
            <div className="p-10 max-w-md mx-auto text-center bg-gray-800 rounded-xl mt-10 shadow-xl">
                ...
                <p className="text-gray-400 mt-2">
                    This admin panel is only accessible to authenticated organizers. Please ensure you are logged in with the appropriate access token.
                </p>
    ...
    return (
        <div className="p-4 md:p-8 max-w-6xl mx-auto">
            ...
            <div className="mt-4 p-4 bg-gray-800 rounded-lg text-gray-400">
                <p>💡 **Admin Tip:** You must use the **Start Editing** and **Save Changes** buttons to update the data. The public scoreboard will reflect changes instantly after you save.</p>
            </div>
        </div>
    );
};
...
// --- PLACEHOLDER COMPONENTS (Results, Gallery, Announcements, Contacts) ---
...
const AppContent = () => {
  ...
  // Apply the custom background image style
  ...
  return (
    <div className={`min-h-screen font-sans text-white`} style={backgroundStyle}>
      ...
      {/* Dark overlay for background image subtlety and text readability */}
